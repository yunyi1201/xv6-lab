...
/*creat kernel pagetable for pre-process   */
pagetable_t 
kvmcreat(void)
{
...
void 
kfreepagetable(pagetable_t kpagetable)
{
	for(int i=0; i<512; i++){
		...
		if(pte & PTE_V){
			...
			if((pte & (PTE_R|PTE_W|PTE_X)) == 0){
				uint64 child = PTE2PA(pte);
				kfreepagetable((pagetable_t)child);	
			}
		} else if(pte & PTE_V){
			panic("kvmfree: leaf\n");
		}
...
static void
walkpage(pagetable_t pg, int level)
{
	char *prefix;
	if(level == 0)
		prefix = "..";
	...
	for(int i=0; i<512; i++){
		...
		if(pte & PTE_V){
			...
			if((pte & (PTE_R|PTE_W|PTE_X)) == 0)
				walkpage((pagetable_t)child, level+1);
		}	
	}
}
...
int 
uvm2k(pagetable_t upg, pagetable_t kpg, uint64 start, uint64 end)
{
	pte_t * u_pte, *k_pte;
	if((end < start) || PGROUNDUP(end) >= PLIC)
		return -1;
	for(uint64 va = PGROUNDUP(start); va < end; va += PGSIZE){
		if((u_pte = walk(upg, va, 0)) == 0)
			panic("copyin_new: pte not exist");
		if((k_pte = walk(kpg, va, 1)) == 0)
			panic("copuin_new: walk failed");
